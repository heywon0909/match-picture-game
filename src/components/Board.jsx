  ...
  useEffect(() => {
    ...
    if (cardArr.length === 2) {
      ...
      if (isSameValue) {
        ...
        if (!answer) {
          let diff = Math.abs(0-(card_1.row)) > Math.abs((boardArr.length -1) - card_1.row)
          if (diff) {
            ...
            if (card1_row === boardArr.length - 1 && card2_row === boardArr.length - 1) {
              console.log('맞았습니다.');
              answer = true;
            }
        ...
        if (!answer) {
          ...
          const solution = () => {
            ...
            const dfs = (x, y, curve,prev_curve) => {
